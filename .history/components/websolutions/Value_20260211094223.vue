<template>
    <section ref="tekfolioEdgeSection" class="w-full bg-gray-100 py-16 lg:py-20 relative overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto px-8 sm:px-12 lg:px-16 relative z-10">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center mb-12 lg:mb-16">
                ...
                <!-- Right: Image (Desktop) / After heading (Mobile) -->
                ...
                <div class="space-y-5">
                    <div v-for="(feature, index) in features.slice(0, 4)" :key="feature.id"
                        ...
                        <button @click="toggleCard(index)"
                            ...
                            <!-- Icon Container -->
                            <div
                                class="shrink-0 w-12 h-12 rounded-xl icon-gradient flex items-center justify-center shadow-sm group-hover:shadow-md group-hover:scale-110 transition-all duration-300">
                                <component :is="feature.icon" class="w-6 h-6 text-gray-900" :stroke-width="2.5" />
                            </div>
            ...
            <div class="grid grid-cols-1 gap-5 lg:hidden">
                <div v-for="(feature, index) in features" :key="feature.id"
                    ...
                    <button @click="toggleCard(index)"
                        ...
                        <!-- Icon Container -->
                        <div
                            class="shrink-0 w-12 h-12 rounded-xl icon-gradient flex items-center justify-center shadow-sm group-hover:shadow-md transition-all duration-300">
                            <component :is="feature.icon" class="w-6 h-6 text-gray-900" :stroke-width="2.5" />
                        </div>
...
/* Light Icon Gradient for Icon Backgrounds */
.icon-gradient {
