    ...
    <div class="min-h-screen bg-white">
        ...
        <section ref="howWeWorkSection" class="w-full bg-gray-100 py-20 lg:py-28 relative overflow-hidden">
            ...
            <div class="max-w-7xl mx-auto px-8 sm:px-12 lg:px-16 relative z-10">
                <!-- Desktop: Two Column Layout with 1/3 - 2/3 split -->
                <div class="hidden lg:grid lg:grid-cols-12 gap-8 mb-4">
                    <!-- Left Column: Section Header (1/3 width) -->
                    ...
                    <!-- Right Column: All Cards (2/3 width) -->
                    <div class="col-span-8 mt-0 lg:mt-16">
                        <div class="grid grid-cols-1 gap-4">
                            <div v-for="(step, index) in steps" :key="step.id" :class="['group bg-gray-800 rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border border-gray-700 hover:scale-[1.02] card-glow',
                                ...
                                <button @click="toggleCard(index)"
                                    ...
                                    <!-- Icon -->
                                    <div
                                        class="flex-shrink-0 w-12 h-12 bg-gradient-to-br from-purple-600/20 to-blue-600/20 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                                        <component :is="step.icon"
                                            class="w-6 h-6 bg-gradient-to-br from-purple-600 to-blue-600 bg-clip-text text-transparent"
                                    ...
                                    <div class="chevron-glow-circle">
                                        <svg :class="['w-5 h-5 transition-transform duration-300 flex-shrink-0 bg-gradient-to-br from-purple-600 to-blue-600 bg-clip-text text-transparent',
                                            { 'rotate-180': openCards[index] }]" fill="none" viewBox="0 0 24 24"
                                            stroke="currentColor"
                                            style="filter: drop-shadow(0 0 4px rgba(147, 51, 234, 0.6));">
                    ...
                    <div v-for="(step, index) in steps" :key="step.id" :class="['group bg-gray-800 rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border border-gray-700 hover:scale-[1.02] card-glow',
                        ...
                        <button @click="toggleCard(index)"
                            ...
                            <div class="chevron-glow-circle">
                                <svg :class="['w-5 h-5 transition-transform duration-300 flex-shrink-0 bg-gradient-to-br from-purple-600 to-blue-600 bg-clip-text text-transparent',
                                    { 'rotate-180': openCards[index] }]" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor" style="filter: drop-shadow(0 0 4px rgba(147, 51, 234, 0.6));">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M19 9l-7 7-7-7" />
                                </svg>
                            </div>
        ...
        <section ref="loveWorkingSection" class="w-full bg-white py-20 lg:py-28 relative overflow-hidden">
            <div class="max-w-7xl mx-auto px-8 sm:px-12 lg:px-16 relative z-10">
                <!-- Two Column Layout on Desktop -->
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
                    <!-- Left Column: Content (Order 1 on mobile and desktop) -->
                    <div class="bg-gray-800 rounded-2xl p-8 lg:p-12 order-1">
                        <!-- Section Header -->
                        <div class="mb-12">
                            <div :class="['flex items-center gap-3 mb-4', { 'animate-slide-in-left': textVisible }]">
                                <div class="h-px w-12 bg-gradient-to-r from-purple-600 to-blue-600"></div>
                                <span
                                    class="text-sm font-semibold bg-gradient-to-r from-purple-600 to-blue-600 bg-clip-text text-transparent uppercase tracking-wider">The
                                    Right
                                    Fit</span>
                            </div>
                            <h2 :class="['text-2xl md:text-3xl lg:text-4xl font-bold text-white leading-tight mb-6',
                                { 'animate-slide-in-left': textVisible }]" style="animation-delay: 50ms;">
                                You'll Love Working With Us If…
                            </h2>
                        </div>

                        <!-- Content List with Check Mark Bullets -->
                        <div class="space-y-6">
                            <div :class="['flex items-start gap-4', { 'animate-slide-in-left': textVisible }]"
                                style="animation-delay: 100ms;">
                                <div class="flex-shrink-0 mt-1">
                                    <div
                                        class="w-6 h-6 rounded-full bg-gradient-to-br from-purple-600 to-blue-600 flex items-center justify-center">
                            ...
                            <div :class="['flex items-start gap-4', { 'animate-slide-in-left': textVisible }]"
                                ...
                                <p class="text-base leading-relaxed text-white">
                                    You want a partner who <span class="font-bold">thinks with you</span>, not just
                                    executes
                                    tasks
                                </p>
                            </div>
...
<script setup>
import { Code, Compass, Palette, Rocket, Search, Headphones } from 'lucide-vue-next';
import { onMounted, onUnmounted, ref } from 'vue';
import howWeWork from '~/assets/img/howwework-tekfolio.svg';

const openCards = ref([false, false, false, false, false, false]);
const windowWidth = ref(0);
const cardsVisible = ref(false);
const textVisible = ref(false);
const howWeWorkSection = ref(null);
const loveWorkingSection = ref(null);
...
const steps = [
    ...
    },
    {
        ...
        icon: Headphones,
        description: "Ongoing support ensures your product stays current, secure, and optimized. We provide maintenance, updates, and enhancements to keep your platform running smoothly."
    }
];
...
const handleScroll = () => {
    ...
    if (loveWorkingSection.value && !textVisible.value) {
        const rect = loveWorkingSection.value.getBoundingClientRect();
        const isVisible = rect.top < window.innerHeight * 0.75;
        if (isVisible) {
...
/* Card glow effect - enhanced for darker cards */
.card-glow {
...
/* Enhanced chevron glow with circle effect */
.chevron-glow-circle {
    position: relative;
    display: inline-flex;
    align-items: center;
