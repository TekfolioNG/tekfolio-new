        ...
        <section ref="howWeWorkSection" class="w-full bg-gray-100 py-20 lg:py-28 relative overflow-hidden">
            ...
            <div class="max-w-7xl mx-auto px-8 sm:px-12 lg:px-16 relative z-10">
                ...
                <div class="hidden lg:grid lg:grid-cols-2 gap-8 mb-8">
                    ...
                    <div class="mt-20">
                        <div :class="['group bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border border-gray-200 hover:scale-[1.02] card-glow',
                            { 'animate-slide-up': cardsVisible }]" style="animation-delay: 0ms;">
                            <!-- Card Header - Clickable -->
                            <button @click="toggleCard(0)"
                                ...
                                <!-- Chevron with glow -->
                                <svg :class="['w-5 h-5 text-purple-600 transition-transform duration-300 flex-shrink-0 chevron-glow',
                                    { 'rotate-180': openCards[0] }]" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor">
                            ...
                            <div :class="['overflow-hidden transition-all duration-300 ease-in-out',
                                openCards[0] ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0']">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-6">
                    ...
                    <template v-if="windowWidth < 1024">
                        <div v-for="(step, index) in steps" :key="step.id" :class="['group bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border border-gray-200 hover:scale-[1.02] card-glow',
                    ...
                    <template v-else>
                        <div v-for="(step, index) in steps.slice(1)" :key="step.id" :class="['group bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border border-gray-200 hover:scale-[1.02] card-glow',
                            { 'animate-slide-up': cardsVisible }]" :style="`animation-delay: ${(index + 1) * 100}ms;`">
                            <!-- Card Header - Clickable -->
                            <button @click="toggleCard(index + 1)"
                                class="w-full p-6 flex items-center gap-4 hover:bg-gray-50 transition-colors duration-200 text-left">
                                <!-- Icon -->
                                <div
                                    class="flex-shrink-0 w-14 h-14 bg-gradient-to-br from-purple-50 to-blue-50 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                                ...
                                <!-- Chevron with glow -->
                                <svg :class="['w-5 h-5 text-purple-600 transition-transform duration-300 flex-shrink-0 chevron-glow',
                                    { 'rotate-180': openCards[index + 1] }]" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M19 9l-7 7-7-7" />
                                </svg>
                            ...
                            <!-- Card Content - Expandable -->
                            <div :class="['overflow-hidden transition-all duration-300 ease-in-out',
                                openCards[index + 1] ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0']">
                                <div class="px-6 pb-6 border-t border-gray-100 pt-4">
                                    <p class="text-base leading-relaxed text-gray-600">
                                        {{ step.description }}
                                    </p>
                    ...
                    <div class="hidden lg:block">
                        <div class="bg-gradient-to-br from-gray-50 to-white rounded-2xl p-12 shadow-sm">
                            <img :src="howWeWork" alt="Tekfolio How We Work"
                                class="w-full h-auto scale-125 transform group-hover:scale-105 transition-transform duration-700 ease-out" />
                        </div>
                    </div>
                </div>
            </div>
...
/* Card glow effect */
.card-glow {
    box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 0 15px rgba(124, 58, 237, 0.1);
...
@keyframes cardPulse {
    ...
    50% {
        box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 0 25px rgba(124, 58, 237, 0.25);
    }
}
