    ...
    <div class="portable-text">
        <template v-for="(block, index) in value" :key="index">
            ...
            <ul v-if="block._type === 'block' && block.listItem === 'bullet'"
                class="list-disc list-outside pl-6 mb-4 space-y-2 text-gray-800 text-base md:text-lg">
            ...
            <ol v-else-if="block._type === 'block' && block.listItem === 'number'"
                class="list-decimal list-outside pl-6 mb-4 space-y-2 text-gray-800 text-base md:text-lg">
                <li class="leading-relaxed">
                    <component v-for="(child, childIndex) in block.children" :key="childIndex"
                        :is="renderChild(child, block.markDefs || [])" />
                </li>
...
const getComponent = (block) => {
    ...
    if (block._type === 'block' && !block.listItem) {
        ...
        switch (style) {
            ...
            case 'blockquote':
                return () => h('blockquote', {
                    class: 'border-l-4 border-purple-600 pl-4 italic my-4 text-gray-900 text-base md:text-lg'
                }, children)
            default:
                return () => h('p', { class: 'mb-4 leading-relaxed text-gray-900 text-base md:text-lg' }, children)
        }
    }
