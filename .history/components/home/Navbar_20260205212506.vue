            ...
            <Transition name="dropdown">
              <div v-if="companyDropdownOpen"
                class="absolute top-full left-0 mt-1 w-56 rounded-lg shadow-xl overflow-hidden backdrop-blur-md" :class="isDarkMode
                  ? 'bg-linear-to-b from-purple-900/98 to-blue-900/98 border border-purple-700/40'
          ...
          <div class="space-y-1">
            ...
            <Transition name="mobile-dropdown">
              <div v-if="mobileCompanyOpen" class="pl-4 space-y-1">
                <NuxtLink v-for="item in companyDropdownItems" :key="item.path" :to="item.path"
                  class="block font-semibold px-4 py-2.5 rounded-lg text-sm transition-all" :class="route.path === item.path
                    ? 'text-white bg-linear-to-r from-purple-700 to-blue-600'
                    : isDarkMode
                      ? 'text-gray-100 hover:text-white hover:bg-white/10'
                      : 'text-gray-700 hover:text-white hover:bg-linear-to-r hover:from-purple-700 hover:to-blue-600'"
